	...
	<div>
		<div class="container">
			<div class="handle-box">
				...
				<el-input v-model="searchDto.unit"  placeholder="单位" class="handle-input mr10"></el-input>
				<el-input v-model="searchDto.community" placeholder="社区" class="handle-input mr10"></el-input>
		...
		<el-dialog title="编辑" v-model="editVisible" width="30%">
			<el-form label-width="70px" ref="ruleFormRef" :rules="rules" :model="formEdit">
					...
					<el-form-item label="社区" prop="community">
						<el-input v-model="formEdit.community"></el-input>
					</el-form-item>
		...
		<el-dialog title="新增" v-model="addVisible" width="30%">
			<el-form label-width="70px" ref="ruleFormRef" :rules="rules" :model="formAdd">
				<el-form-item label="姓名" prop="name">
					...
					<el-form-item label="单位" prop="unit">
						<el-input v-model="formAdd.unit"></el-input>
					</el-form-item>
					<el-form-item label="社区" prop="community">
						<el-input v-model="formAdd.community"></el-input>
					</el-form-item>
					<el-form-item label="住址" prop="address">
...
import { DoctorInfoApi, DoctorInfoDto, DoctorInfoDtoPagedListDto, GetPagedListDto, PatientInfoApi, PatientInfoCreateDto, PatientInfoDto, PatientInfoUpdateDto } from "../api-services";
...
const rules = reactive<FormRules>(
	{
		...
		unit: [{ required: true, message: "输入不为空!", trigger: 'blur' }],
		community: [{ required: true, message: "输入不为空!", trigger: 'blur' }],
...
const tableData = ref<PatientInfoDto[]>();
...
const getData = async () => {
	...
	const [err, res] = await feature(getAPI(PatientInfoApi)
		.apiPatientInfoGetPagedListPost(getPagedListDto));
	if (err) {
		console.log(err);
	} else {
...
// 查询操作
const searchDto =ref<PatientInfoDto>({});
// 查询操作
...
const handleDelete = async (index: number) => {
	...
	ElMessageBox.confirm('确定要删除吗？', '提示', {
		...
		const [err, res] = await feature(getAPI(PatientInfoApi)
			.apiPatientInfoDeleteIdDelete(id));
		if (err) {
			console.log(err);
		} else {
...
let formEdit = ref<PatientInfoUpdateDto>({
});
let formAdd = ref<PatientInfoCreateDto>({});
let idx: number = -1;
...
const saveAdd = async (formEl: FormInstance | undefined) => {
	...
	await formEl.validate(async (valid, fields) => {
		if (valid) {
			const [err, res] = await feature(getAPI(PatientInfoApi)
				.apiPatientInfoInsertPost(formAdd.value));
			if (err) {
...
const saveEdit = async (formEl: FormInstance | undefined) => {
	...
	await formEl.validate(async (valid, fields) => {
		if (valid) {
			const [err, res] = await feature(getAPI(PatientInfoApi)
				.apiPatientInfoUpdatePut(formEdit.value));
			if (err) {
				console.log(err);
			} else {
